...
def read_correction(file_in, tokenizer, max_seq_length=1024):
    i_line = 0
    pos_samples = 0
    neg_samples = 0
    features = []
    ...
    for sample in tqdm(data, desc="Example"):
        ...
        n_claim_ners = sample['n_claim_ner']
        ...
        for h, t in train_triple.keys():
            ...
            hts.append([h, t])
            pos_samples += 1
        for h in range(n_claim_ners):
            for t in range(n_claim_ners, len(entities)):
        ...
        assert len(relations) == n_claim_ners*(len(entities) - n_claim_ners)
...
def read_correction_old(file_in, tokenizer, max_seq_length=1024):
    i_line = 0
    pos_samples = 0
    neg_samples = 0
    features = []
